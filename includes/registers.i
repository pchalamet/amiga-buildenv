...
;*--------------------------------*
;* hardware registers AGA chipset *
;*--------------------------------*
_CustomBase=$dff000
bltddat=$000
dmaconr=$002
...
KILL_SYSTEM	macro
Kill_System
	IFEQ (NARG=1)|(NARG=2)			; mandatory parameters
	FAIL Missing parameters !!!
	ENDC
	bra.s .Skip
	dc.b "$VER: (c)1995 Sync of DreamDealers"
.GfxName
...
.from_CLI
;	move.l a3,a1				baisse notre priorit�...
;	moveq #-128,d0
;	CALL SetTaskPri
;
;	lea .VillageName(pc),a1			ouvre la village.library si pr�sente
;	moveq #0,d0
...
.Gfx
	...
	IFNE NARG=2				;ya un 2�me parametre ?
	IFNE \2					;c'est egal � 0 ?
...
.delay	CALL WaitTof
	...
	ENDC
	ELSEIF
	moveq #2*50-1,d7			;pas de 2�me parametre => 2 sec
.delay	CALL WaitTof				;d'attente
	...
	move.w #$ff00,potgo(a5)			;devraient l'�tre
	move.b ciaapra,-(sp)
	move.b ciaaddra,-(sp)
	move.b #$3,ciaaddra
...
.clear	clr.l (a0)+
	...
	movem.l a5/a6,-(sp)			;saute � la routine pass�e comme
	lea \1(pc),a0				;param�tre de KILL_SYSTEM
	move.l a0,$80.w
	...
	CALL WaitBlit				;lib�re le blitter
	CALL DisownBlitter

	move.l (sp)+,$80.w			;remet tout comme c'�tait avant
	move.l (sp)+,$78.w
	move.l (sp)+,$6c.w
	move.l (sp)+,$68.w
