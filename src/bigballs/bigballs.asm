...
* utilise 16 triangles de 256 pixels de base. chaque triangle est d�cal�
* de 1 pixel � droite par rapport au pr�c�dent.
* => 32 octets pour le triangle
* => 02 octets pour le shift � droite
* => 44 octets pour le d�but ou la fin de l'image
* => 44 octets suppl�mentaire avant le triangle pour pouvoir afficher � droite
...
* dc.l offset par rapport � la ligne d'avant
* dc.l offset par rapport � la ligne d'avant
* ....
...
*-----------------------> zoupla... viva mes bolla !!!
	section neant,code_c

	KILL_SYSTEM BigBalls
	moveq #0,d0
	rts
...
build_others
	...
	moveq #15,d1				un decalage de 1 pixel �
	bra.s wait_dup_triangle			chaque fois

loop_all_triangle
	lea (32+2+44)*129(a4),a4
	move.l #triangle_area,bltapt(a6)
	move.l a4,bltdpt(a6)
	clr.l bltamod(a6)
...
do_Alpha
	...
	bgt.s Alpha_test			signe du r�sultat
	beq.s do_Teta
	add.w #360*4,-2(a0)
	bra.s do_Teta
Alpha_test
...
do_Teta
	...
	bgt.s Teta_test				signe du r�sultat
	beq.s do_Phi
	add.w #360*4,-2(a0)
	bra.s do_Phi
Teta_test
...
do_Phi
	...
	bgt.s Phi_test				signe du r�sultat
	beq.s end_Angles
	add.w #360*4,(a0)
	rts
Phi_test
...
Compute_Matrix
	...
	move.w sinteta,d6
	neg.w d6
	asr.w #1,d6				on perd un bit � cause du swap
...
small_ptr
	...
	section ballon,bss_c
triangle_area
	ds.b (32+2+44)*129*16+4
coplist1
	ds.b COPLIST_SIZE
coplist2
	ds.b COPLIST_SIZE
